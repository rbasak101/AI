...
# Transform between alien configs and an array index| alien position is converted to index
def configToIdx(config, offsets, granularity,alien):   # config: (x, y, shape)
    result = []
    for i in range(len(config[:2])):
        result.append(int((config[i]-offsets[i]) / granularity))
    result.append(alien.get_shapes().index(config[-1]))
    print("Inside: configToIdx ", tuple(result))
    return tuple(result)

def idxToConfig(index, offsets, granularity,alien): # maze index that is converted to alien position
    result = []
    for i in range(len(index[:2])):
        result.append(int((index[i]*granularity)+offsets[i]))
    result.append(alien.get_shapes()[index[-1]])
    print("Inside: idxToConfig ", tuple(result))
    return tuple(result)
